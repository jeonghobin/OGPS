      ...
      <div class="row justify-content-between ps-2 pe-2 ms-2 ms-2">
        <div class="col-auto">
          <a type="button" id="write-button" class="btn btn-outline-primary" style="font-weight: bold"
            @click="$router.push('/freeboard/write')"> 글쓰기</a>
        </div>
        <div class="col-auto">
...
import BoardListItem from "@/components/freeboard/BoardListItem.vue";
...
export default {
  ...
  created() {
    // 비동기
    // TODO : 글목록 얻기.
    http.get(`/api/free?pgno=1&key=&word=`).then((response) => {
  ...
  methods: {
    ...
    searchList(){
        http.get(`/api/free?pgno=1&key=${this.key}&word=${this.search}`).then((response) => {
        console.log(response)
        this.navigator = response.data.navigation.navigator;
        this.articles = response.data.posts;
        this.pgno = response.data.pgno;
