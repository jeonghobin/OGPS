...
export default {
  ...
  created() {
    ...
    http.get(`/api/free/${this.articleno}`).then((response) => {
  ...
  methods: {
    moveModifyArticle() {
      ...
      this.$router.push({ name: "freeboardmodify", params: { articleno: this.article.article_no } });
    },
    deleteArticle() {
      console.log("글삭제 하러가자!!!");
      http.delete(`/api/free/${this.articleno}`).then((response) => {
        console.log(response);
      })
      .then(()=>{
